...
int main(int argc, char* argv[])
{
	...
	devs = hid_enumerate(0x77d, 0x0410);
	cur_dev = devs;
	while (cur_dev) {
		printf("Device Found\n  type: %04hx %04hx\n  path: %s\n  serial_number: %ls",
	...
	handle = hid_open(0x77d, 0x0410, NULL);
	...
	buf[1] = 0x06; // 6 bytes in report 
	//first two bytes input, third is unused
	buf[5] = 0x0;  // LED off
	buf[6] = pulse_state.status; // Status defined above
	buf[7] = 0x01; // multiplier?
	...
	// // Send an Output report to toggle the LED (cmd 0x2)
